...
CREATE TABLE IF NOT EXISTS `scrumlab`.`admins` (
  ...
  `enable` TINYINT(1) GENERATED ALWAYS AS (true)  COMMENT 'Czy jest aktywny i może się logować.',
  PRIMARY KEY (`id`))
ENGINE = InnoDB
...
CREATE TABLE IF NOT EXISTS `scrumlab`.`day_name` (
  ...
  `order` INT NULL COMMENT 'Kolejność.',
  PRIMARY KEY (`id`))
ENGINE = InnoDB
...
CREATE TABLE IF NOT EXISTS `scrumlab`.`recipe_plan` (
  ...
  `order` INT NULL COMMENT 'Kolejność posiłku w ciągu dnia.',
